...
def submit_form():
    ...
    def save_to_db():
        try:
            ...
            crs = conn.cursor()
            crs.execute("INSERT INTO demographics (lastname, firstname, agegroup, educationgroup, ethnicitygroup, politicalgroup, incomegroup, createdt) \
                        VALUES(?,?,?,?,?,?,?,?)", (last_name, first_name, age_val, education_val, ethnicity_val, party_val, income_val, datetime.now()))
    ...
    if has_numbers(last_name) or has_numbers(first_name):
        ...
        if save_to_db():
            messagebox.showinfo(title="Saved To Database", message="Input data was saved to database successfully")
...
frm7 = tk.Frame(win, relief="sunken", bd=2)
frm7.grid(row=8, column=0, padx = 10, pady=10, columnspan=5, sticky="w")
jpg = PhotoImage(file="C:\\Users\\17147\\Desktop\\CMPR114\\m8\\wolverine25.png")
lblImage = tk.Label(frm7, image=jpg)
